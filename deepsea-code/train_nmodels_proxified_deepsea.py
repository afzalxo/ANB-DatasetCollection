...
from trainval.trainval_deepsea import train_x_epochs
from dataloader.deepsea_dataloaders import deepsea_dataloader_tv#DeepSEAFFCVDataLoader
from auxiliary.utils import CrossEntropyLabelSmooth, create_optimizer, setup_for_distributed
from models.accelbenchnet_1d_deepsea import AccelNet1d as Network
...
def profile_model(input_size, design, activation_fn, mode, local_rank):
    model_temp = Network(design=design, activation_fn=activation_fn, mode=mode, num_classes=36)
...
def main():
    ...
    if args.use_wandb:
        ...
        wandb_con = wandb.init(
            ...
            name=args.note + f"_{args.job_id}_bs{args.train_batch_size}_ep{args.epochs}_seed{args.seed}_rank{args.local_rank}",
            settings=wandb.Settings(code_dir="."),
            dir=wandb_metadata_dir,
            group='junk_rebuttal_deepsea'
        )
    ...
    num_models = 40
    job_idx = 0
    job_offset = job_idx * num_models * args.world_size
    m = job_offset + args.local_rank * num_models
    starting_index = m

    # FFCV loader here
    # dl = DeepSEAFFCVDataLoader(local_rank, args.num_workers)
    ...
    dl = None
    train_queue, valid_queue = deepsea_dataloader_tv("/home/aahmadaa/datasets/deepsea/", args.train_batch_size)
    ...
    while m < starting_index + num_models:# len(archs):
        args.model_num = m
        args.design = archs[m][0]# searchables.Searchables().random_searchable()# archs[m][0]
        logging.info(
        ...
        '''
        model = Network(design=args.design, activation_fn=activation_fn, mode=mode, num_classes=36)
        ...
        logging.info(
            "Job ID: %d, Model Number: %f, Train Time: %f, Model AUROC: %f",
            args.job_id,
            args.model_num,
            train_time,
            valid_t1,
        ...
        archs[m].append(valid_t1)
        with open(f'results_deepsea/deepsea_160_ep{args.epochs}_bs{args.train_batch_size}_start{starting_index}_num{num_models}_seed{args.seed}.tor', 'wb') as fh:
            arch_d = {"arch_dict": archs, "batch_size": args.train_batch_size, "epochs": args.epochs}
