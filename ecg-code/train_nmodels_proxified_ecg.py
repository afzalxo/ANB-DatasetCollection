...
from trainval.trainval_ecg import train_x_epochs
from dataloader.ecg_dataloaders import ecg_dataloader_tv
from auxiliary.utils import CrossEntropyLabelSmooth, create_optimizer, setup_for_distributed
from models.accelbenchnet_1d_ecg import AccelNet1d as Network
...
def main():
    ...
    dl = None
    train_queue, valid_queue, dl = ecg_dataloader_tv("/home/aahmadaa/datasets/ecg/", args.train_batch_size)
    ...
    while m < starting_index + num_models:# len(archs):
        args.model_num = m
        args.design = searchables.Searchables().random_searchable()# archs[m][0]
        logging.info(
        ...
        '''
        model = Network(design=args.design, activation_fn=activation_fn, mode=mode, num_classes=4)
        ...
        logging.info(
            "Job ID: %d, Model Number: %f, Train Time: %f, Model F1 Score: %f",
            args.job_id,
            args.model_num,
            train_time,
            valid_t1,
        ...
        with open(f'results_ecg/ecg_160_ep{args.epochs}_bs{args.train_batch_size}_start{starting_index}_num{num_models}_seed{args.seed}.tor', 'wb') as fh:
